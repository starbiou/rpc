...
func (rq *ReceiptQuery) QueryItems() *ItemQuery {
...
func (rq *ReceiptQuery) WithItems(opts ...func(*ItemQuery)) *ReceiptQuery {
	query := (&ItemClient{config: rq.config}).Query()
...
func (rq *ReceiptQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Receipt, error) {
	...
	if query := rq.withItems; query != nil {
		if err := rq.loadItems(ctx, query, nodes,
			func(n *Receipt) { n.Edges.Items = []*Item{} },
			func(n *Receipt, e *Item) { n.Edges.Items = append(n.Edges.Items, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReceiptQuery) loadItems(ctx context.Context, query *ItemQuery, nodes []*Receipt, init func(*Receipt), assign func(*Receipt, *Item)) error {
	...
	query.Where(predicate.Item(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(receipt.ItemsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.receipt_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "receipt_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "receipt_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
