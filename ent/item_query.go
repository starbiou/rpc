...
//
//	client.Item.Query().
//		GroupBy(item.FieldShortDescription).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Item.Query().
//		Select(item.FieldShortDescription).
//		Scan(ctx, &v)
...
func (iq *ItemQuery) loadReceipt(ctx context.Context, query *ReceiptQuery, nodes []*Item, init func(*Item), assign func(*Item, *Receipt)) error {
	...
	for i := range nodes {
		if nodes[i].receipt_items == nil {
			continue
		}
		fk := *nodes[i].receipt_items
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "receipt_items" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
