...
func (s *ReceiptService) AddReceipt(ctx context.Context, receipt schemas.Receipt) (string, error) {
	return s.repo.AddReceipt(ctx, receipt)
}
...
func (s *ReceiptService) CalculatePoints(receipt models.Receipt) int {
	...
	points += s.calculateTotalPoints(receipt.Total)
	points += s.calculateItemPoints(receipt.Items)
...
func (s *ReceiptService) calculateRetailerNamePoints(retailer string) int {
	...
	return alphanumericCount * RetailerNamePointsMultiplier
}
...
func (s *ReceiptService) calculateItemPoints(items []models.Item) int {
	points := (len(items) / 2) * TwoItemsPoints
	for _, item := range items {
		if len(strings.TrimSpace(item.ShortDescription))%3 == 0 { // Rule 4: Every two items
			points += int(item.Price*DescriptionLengthMultiplier + 0.5) // Rule 5: Description length multiple of 3
		}
	}
	return points
}
